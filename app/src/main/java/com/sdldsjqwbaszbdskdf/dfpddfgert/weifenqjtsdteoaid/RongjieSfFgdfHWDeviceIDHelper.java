...
public class RongjieSfFgdfHWDeviceIDHelper {
    private Context mContext;
    public final LinkedBlockingQueue<IBinder> linkedBlockingQueue = new LinkedBlockingQueue(1);

    public RongjieSfFgdfHWDeviceIDHelper(Context ctx) {
        mContext = ctx;
    }

    public void getHWID(RongjieSfFgdfDevicesIDsHelper.AppIdsUpdater _listener) {
        try {
        ...
        if (isBin) {
            try {
                ...
                RongjieSfFgdfHWIDInterface.RongjieSfFgdfHWID hwID = new RongjieSfFgdfHWIDInterface.RongjieSfFgdfHWID(iBinder, mContext);
