...
import com.qwsdbsdvdsnd.dfmcvndfhdfsdfwe.api.ApiManager;
...
import com.qwsdbsdvdsnd.dfmcvndfhdfsdfwe.util.DownloadUtil;
...
public class ProductDetailsActivity extends RxAppCompatActivity implements EasyPermissions.PermissionCallbacks {
    ...
    private void getConfig() {
        ApiManager apiService = NetSimple.getRetrofitManager().getApiService();  //传入之前定义的接口
        apiService.getValue("VIDEOTAPE").enqueue(new Callback<BaseModel<ConfigModel>>() {
            @Override
            public void onResponse(Call<BaseModel<ConfigModel>> call, Response<BaseModel<ConfigModel>> response) {
                ...
                if (model != null){
                    SPUtil.saveBool("NO_RECORD", !model.getVideoTape().equals("0"));
